...
const host = 'https://talking.onrender.com'
...
function App() {
  ...
  async function chat(id, text) {
    const res = await axios.post(
      `https://architect-n16u.onrender.com/api/v1/homes/${id}/chat`,
      {
        chat: text,
      }
    );
    setText(res.data.chat);
    setSpeak(true);
  }
  ...
  return (
    <div className="full">
      <div style={STYLES.area}>
        <textarea rows={4} type="text" style={STYLES.text} value={text} onChange={(e) => setText(e.target.value.substring(0, 2000))} />
        <button onClick={() => chat(query, text)} style={STYLES.speak}> { speak? 'Running...': 'Speak' }</button>
